...
export const ApplicantDetailsModal: React.FC<ApplicantDetailsModalProps> = ({
  ...
  const handleViewResume = async () => {
    ...
    try {
      ...
      const token = await currentUser.getIdToken();
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://skillsync-backend-gwwo.onrender.com/api'}/resumes/view/${applicationData._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleDownloadResume = async () => {
    try {
      ...
      const token = await currentUser.getIdToken();
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://skillsync-backend-gwwo.onrender.com/api'}/resumes/download/${applicationData._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const saveNotes = async () => {
    try {
      ...
      const token = await currentUser.getIdToken();
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://skillsync-backend-gwwo.onrender.com/api'}/applications/${applicant.id}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
