...
const CreateResumeTab: React.FC<CreateResumeTabProps> = ({ 
  ...
  const loadExistingResumeData = async () => {
    try {      if (!auth.currentUser) {        return;
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://skillsync-backend-gwwo.onrender.com/api'}/resumes/current`, {
        method: 'GET',
        headers: {
  ...
  const uploadOriginalResumeToCloud = async (file: File): Promise<{ success: boolean; data?: { cloudUrl: string; publicId: string }; message?: string }> => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://skillsync-backend-gwwo.onrender.com/api'}/jobseekers/upload-original-resume`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const saveResumeToDatabase = async (resumeData: ResumeData, pdfBase64: string, uploadedResumeUrl: string | null = null) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://skillsync-backend-gwwo.onrender.com/api'}/resumes/create`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
